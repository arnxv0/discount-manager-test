...
export function ApplyDiscountToTag() {
  ...
  async function applyDiscountsToTags() {
    ...
    let response = await fetch("/api/discountTags", {
      method: "POST",
      headers: {
  ...
  async function resetDiscountsTags() {
    ...
    setIsLoading(true);
    let response = await fetch("/api/resetDiscountTags", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  return (
    <>
      {toastMarkup}
      <Card
        title="Apply discount to all products with a tag"
        sectioned
        primaryFooterAction={{
          content: "Apply discount to tag",
          onAction: applyDiscountsToTags,
          loading: isLoading,
        }}
        secondaryFooterActions={[
          {
            content: "Remove discount from tag",
